...
from std_msgs.msg import Float32MultiArray, Float32, Int8MultiArray, Float64MultiArray, Float64
...
class WalkingPolicyControlNode(Node):
    def __init__(self):
        ...
        self.contact_states_subscription = self.create_subscription(
            Float32MultiArray, "/contact_states", self.contact_states_callback, 10
        )
    ...
    def setup_buffer(self):
        ...
        self.commands[0] = 0.5  # Forward velocity
    ...
    def height_data_callback(self, msg):
        self.latest_height_data = msg.data

    def odom_callback(self, msg):
        self.latest_linear_velocity = [msg.twist.twist.linear.x, msg.twist.twist.linear.y, msg.twist.twist.angular.z]
        self.latest_base_gravity = [msg.pose.pose.position.x, msg.pose.pose.position.y, msg.pose.pose.position.z]
    ...
    def preprocess_input(self):
        ...
        if (
            self.latest_linear_velocity is None
        ...
        self.high_level_state.update(
            base_linear_velocity=self.latest_linear_velocity,
    ...
    def postprocess_output(self, output):
        self.previous_output = self.previous_output if torch.isnan(output).any() else output.clone() # Store the output for the next iteration
